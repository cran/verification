...
table.stats<- function(obs, pred = NULL, fudge = 0.01, silent = FALSE) {
    ...
    if(!is.null(pred)& !is.null(obs)){
      ...
      a <- tryCatch(tab.out["1", "1"], error=function(e) 0) + fudge
      b <- tryCatch(tab.out["0", "1"], error=function(e) 0) + fudge
      c <- tryCatch(tab.out["1", "0"], error=function(e) 0) + fudge
      d <- tryCatch(tab.out["0", "0"], error=function(e) 0) + fudge
      # a <-tab.out["1","1"]
      # b <-tab.out["0","1"]
      # c <-tab.out["1","0"]
      # d <-tab.out["0","0"]
